...
function buildAboutView(container: any): void {
  ...
  const subtitle = Text("A native Windows JSON viewer")
  ...
  const tech = Text("Built with Perry — TypeScript compiled to native x86_64")
...
function rebuild(): void {
  ...
  if (currentData === null && parseOk) {
    if (showAbout) {
      ...
      const emptyMsg = Text("Paste JSON with Ctrl+V or load test with Ctrl+L")
...
function handleCollapseAll(): void {
  console.log("Ctrl+2: collapse all")
  resetExpanded()
  rebuild()
}

function handleAbout(): void {
  console.log("Ctrl+I: about")
  showAbout = true
  currentData = null
  parseOk = true
  hideSearchBar()
